...
subroutine sub1 (fff,e_s,var1,var2,var3)
  ...
  implicit none
  integer,parameter :: vecsize = 1048576
  integer,parameter :: vecn = 1048576
...
!  vecn = vecsize
  ...
  print*, 'check point 1'
!x$omp target teams distribute parallel do collapse(3) 
    do k=1,8
      do j=1,3
        do i=1,vecn
          glo6(i,j,k) = 0.0d0
...
!$omp&       )
!$omp teams num_teams(vecn)  thread_limit(0) private(scal3, scal2) 
!x$omp distribute parallel do private(scal3)
!$omp distribute 
  do i=1,vecn
...
!x$omp distribute parallel do private(scal2)
!$omp distribute 
    do i=1,vecn
